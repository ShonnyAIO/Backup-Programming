...
struct Hasher{
  ...
  int get_hashcode(int x){
    ...
    if (i==codes.end()){
      int z; do z=(rand()%(lots-1))+1; while (used.count(z));
      used.insert(z);
      return codes[x]=z;
    }
    return i->second;
...
struct HashCollection{
  uint32_t cur_hash=0;
  void increase(uint32_t x,Hasher &hasher) {cur_hash=(cur_hash+hasher.get_hashcode(x));      if (cur_hash>=lots) cur_hash-=lots;}
  void decrease(uint32_t x,Hasher &hasher) {cur_hash=(cur_hash-hasher.get_hashcode(x)+lots); if (cur_hash>=lots) cur_hash-=lots;}
};
...
vector<SeveralHashCollections<5>> vertex_collection;
...
Label calc_label(int x,int p=-1){
  ...
  for (int i=vertex_edge[x].size(); i--;)
    if (vertex_edge[x][i]==p)
      swap(vertex_edge[x][i],vertex_edge[x][0]);
  ...
  bool rm_parent=(p!=-1 and vertex_edge[x].size()==0);
