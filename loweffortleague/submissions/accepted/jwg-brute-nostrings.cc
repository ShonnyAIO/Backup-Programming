...
struct Bribe {
	...
	int skill;
	long long bribe;
	Bribe(int t, int s, long long b): team(t), skill(s), bribe(b){
	}
};

long long team_bribe(int winner_skill, int loser_skill) {
...
vector<Bribe> calc_bribes(vector<int> t, int s, int e) {
	if (e - s > 1) {
		...
		if (s == 0) {
			return vector<Bribe> {Bribe(0, t[0], team_bribe(t[0], t[1]))};
		} else {
			return vector<Bribe> {Bribe(s, t[s], team_bribe(t[s], t[s+1])), Bribe(s+1, t[s+1], team_bribe(t[s+1], t[s]))};
		}
	}
}
...
int main(){
	...
	Bribe min_bribe = Bribe(-1, -1, 1ll<<55ll);
